...
const { initGridFS, getGridFSBucket, getGridFSFilesCollection } = require('../utils/gridfs');
...
const getFileKind = (file) => {
  ...
  if (file.mimetype.startsWith('image/') || ['.jpg', '.jpeg', '.png', '.webp', '.gif'].includes(extension)) {
    return 'image';
  }
...
const isOwner = (manga, userId) => {
  const ownerId = manga.owner && manga.owner._id ? manga.owner._id : manga.owner;
  return Boolean(ownerId && ownerId.toString() === userId);
};

const canModerate = (user) => Boolean(user && ['moderator', 'admin'].includes(user.role));
...
const createManga = async (req, res, next) => {
  try {
    ...
    if (!title || !description || !genre || !chapters || !releaseYear) {
      return res.status(400).json({ message: 'title, genre, chapters, description, releaseYear are required' });
    }
    ...
    const created = await Manga.findById(manga._id)
      .populate('owner', 'username role');
...
const updateManga = async (req, res, next) => {
  try {
    ...
    if (!isOwner(manga, req.user.id) && req.user.role !== 'admin') {
      return res.status(403).json({ message: 'Only owner or admin can update manga' });
    }
...
const uploadMangaFile = async (req, res, next) => {
  try {
    ...
    if (!req.file) {
      return res.status(400).json({ message: 'File is required. Use multipart/form-data with field "file"' });
    }
    ...
    if (!kind) {
      return res.status(400).json({ message: 'Unsupported file type. Allowed: zip, pdf, images' });
    }
    ...
    if (kind === 'image' && pageNumber === null) {
      const imageFiles = manga.files.filter((file) => file.kind === 'image' && file.pageNumber);
      const maxPage = imageFiles.length ? Math.max(...imageFiles.map((file) => file.pageNumber)) : 0;
      pageNumber = maxPage + 1;
    }

    if (kind === 'image' && pageNumber !== null) {
      const pageAlreadyExists = manga.files.some((file) => file.kind === 'image' && file.pageNumber === pageNumber);

      if (pageAlreadyExists) {
        return res.status(409).json({ message: `Image page ${pageNumber} already exists` });
      }
    }
    ...
    return res.status(201).json({
      message: 'File uploaded successfully. Manga status set to pending moderation',
      status: manga.status,
...
const getMangaPages = async (req, res, next) => {
  try {
    ...
    if (!isPublic && !canAccessPrivateManga(req.user, manga)) {
      return res.status(403).json({ message: 'Not allowed to access files for this manga' });
    }
    ...
    const fileMetadata = manga.files.find((file) => file.fileId.toString() === fileId);
    ...
    const { bucket, filesCollection } = getGridFSResources();
    const storedFile = await filesCollection.findOne({ _id: fileMetadata.fileId });

    if (!storedFile) {
      return res.status(404).json({ message: 'Stored file does not exist in GridFS' });
    }

    res.setHeader('Content-Type', storedFile.contentType || fileMetadata.contentType || 'application/octet-stream');

    if (download === 'true') {
      res.setHeader('Content-Disposition', `attachment; filename="${sanitizeFilename(fileMetadata.originalName)}"`);
    }
...
const getPendingManga = async (req, res, next) => {
  try {
    if (!canModerate(req.user)) {
      return res.status(403).json({ message: 'Only moderator or admin can view pending manga' });
    }
...
const approveManga = async (req, res, next) => {
  try {
    if (!canModerate(req.user)) {
      return res.status(403).json({ message: 'Only moderator or admin can approve manga' });
    }
...
const rejectManga = async (req, res, next) => {
  try {
    if (!canModerate(req.user)) {
      return res.status(403).json({ message: 'Only moderator or admin can reject manga' });
    }
